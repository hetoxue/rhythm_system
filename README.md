...
4. 配置数据库连接（`config.php`）：
   - **手动修改`config.php`文件中的数据库连接信息**（主机、端口、数据库名、用户名、密码）
   - 确保文件权限正确（Linux：chmod 666 config.php）
5. 设置文件权限
6. 配置Nginx/Apache
7. 访问网站完成安装
...
#### 修复
- 修复商品购买`product_orders`表字段不匹配问题
- 修复`pay_method`字段类型错误（字符串转整数）
- 添加商品订单`order_no`自动生成
- 修复商品信息查询JOIN逻辑
- 修复Modal关闭时的闪烁问题
- 修复管理后台支付方式显示逻辑
